...
<body class="bg-background text-on-background font-sans">
    ...
    <!-- Side Navigation Bar -->
    <aside class="flex flex-col h-screen w-64 fixed left-0 top-0 z-40 bg-primary border-r border-outline-variant">
        <div class="p-6 flex flex-col items-center border-b border-on-primary/10">
        ...
        <nav class="flex-1 mt-6 px-2 overflow-y-auto custom-scrollbar space-y-1">
            ...
            @can('staff')
            <!-- Enrollment & Grades (admin + teacher) -->
    ...
    <!-- Top App Bar -->
    <header class="flex justify-between items-center h-16 w-[calc(100%-16rem)] ml-64 px-10 sticky top-0 z-30 bg-surface-container-lowest border-b border-outline-variant shadow-sm">
        <div class="flex items-center gap-4">
            <span class="text-sm font-bold text-primary">{{ setting('school_name', 'ວິທະຍາໄລຄູສົງ ອົງຕື້') }}{{ setting('school_name_en') ? ' | ' . setting('school_name_en') : '' }}</span>
        </div>
        <div class="flex items-center gap-6">
            <div class="relative hidden lg:block">
                ...
                <input class="pl-10 pr-4 py-1.5 bg-surface-container rounded-full border-none focus:ring-2 focus:ring-primary w-64 text-sm" 
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity">
                    <img class="w-8 h-8 rounded-full border border-primary/20" 
    ...
    <!-- Main Content -->
    <main class="ml-64 p-10 bg-background min-h-[calc(100vh-4rem)]">
        {{ $slot ?? '' }}
        @yield('content')
    </main>
